...
**Origin checks.** Browsers send an `Origin` header on the WebSocket upgrade, and any site the user visits can try to open a socket to your server (cross-site WebSocket hijacking). `NetworkWebSocketOptions.AllowedOrigins` / `AllowMissingOrigin` exist, but check that your Nalix version actually enforces them on the upgrade before you rely on them. Until then, enforce the allowlist at the reverse proxy and require application-level auth after the handshake. The handshake encrypts the connection. It does not tell you who the user is.
...
**Per-IP limits during development.** Every tab, reload, and reconnect comes from `127.0.0.1`. The default `ConnectionQuotaOptions` (10 attempts per 5 s, halved in burst mode) will **ban loopback for minutes** after a few quick reloads, and the browser only reports `ERR_CONNECTION_RESET`. The sample raises the limits on the live options object before `Build()`. In production, behind a proxy, configure `TrustedProxyOptions` so that limits apply to real client IPs rather than to the proxy.
